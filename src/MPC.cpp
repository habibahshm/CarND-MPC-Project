...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for(int i =0 ; i < N-1; i++){
      AD<double> x1 = vars[x_start+1+i];
      AD<double> y1 = vars[y_start+1+i];
      AD<double> psi1 = vars[psi_start+1+i];
      AD<double> v1 = vars[v_start+1+i];
      AD<double> cte1 = vars[cte_start+1+i];
      AD<double> epsi1 = vars[epsi_start+1+i];
      ...
      fg[epsi_start+i+2] = epsi1 - ((psi0 - psides0) - v0 / Lf * delta0 * dt);
